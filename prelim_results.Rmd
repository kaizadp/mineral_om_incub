...
gg_vankrev(data_hcoc %>% filter(Pre_post=="Initial") %>% filter(n>1), aes(x = OC, y = HC, color = Clay))+
  ggtitle("initial")+
  stat_ellipse()+
  facet_grid(Temperature~Moisture)+
  theme_kp()+
...
gg_vankrev(data_hcoc %>% filter(Pre_post=="Final") %>% filter(n>1), aes(x = OC, y = HC, color = Clay))+
...
a = data_hcoc %>% filter(Pre_post=="Final") %>% filter(n>1)
```
